...
export default function PostForm({
  ...
  return (
    <form
      ...
      noValidate
      className="flex flex-1 flex-col gap-3 overflow-y-auto px-3.5 py-3"
    >
      <div>
        <label
          htmlFor="community-name"
          className="mb-1 block font-display text-[9px] text-os-ink"
        >
          Name
        </label>
        <input
          ...
          aria-describedby={
            fieldErrors.name ? "community-name-error" : undefined
          }
        ...
        {fieldErrors.name && (
          <p
            ...
            role="alert"
            className="mt-1 font-mono text-[9px] text-os-danger"
          >
            {fieldErrors.name}
          </p>
        )}
      ...
      <div className="flex min-h-0 flex-1 flex-col">
        <div className="mb-1 flex items-baseline justify-between gap-2">
          <label
            ...
            className="block font-display text-[9px] text-os-ink"
          >
            Message
          </label>
        ...
        <textarea
          ...
          aria-describedby={
            fieldErrors.message ? "community-message-error" : undefined
          }
          className="min-h-27.5 w-full min-w-0 flex-1 resize-none rounded-lg border-2 border-os-border bg-os-surface px-2.5 py-1.5 font-mono text-[11px] leading-[1.5] text-os-ink outline-none placeholder:text-os-ink-soft focus:border-os-accent disabled:opacity-60"
        />
        {fieldErrors.message && (
          <p
            id="community-message-error"
            role="alert"
            className="mt-1 font-mono text-[9px] text-os-danger"
          >
            {fieldErrors.message}
      ...
      <p className="rounded-lg border-2 border-os-border bg-os-surface-2 px-2.5 py-2 font-mono text-[9px] leading-relaxed text-os-ink-soft">
        Be kind and respectful. Let&rsquo;s keep the BrowserOS community a great
        place.
      </p>
      ...
      {submitError && (
        <p
          role="alert"
          className="rounded-lg border-2 border-os-danger bg-os-surface px-2.5 py-2 font-mono text-[9px] leading-relaxed text-os-danger"
        >
          {submitError.message}
        </p>
      )}
      ...
      <button
        type="submit"
        disabled={submitting}
        className="pixel-cut mt-1 inline-flex items-center justify-center border-(length:--os-border-width) border-os-border-strong bg-os-accent px-3 py-2 font-display text-[10px] text-os-accent-ink shadow-[0_2px_0_rgba(0,0,0,.15)] transition-transform hover:-translate-y-0.5 active:translate-y-0 disabled:cursor-wait disabled:opacity-60 disabled:hover:translate-y-0"
      >
